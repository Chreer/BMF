...
class Runner(object):
    def __init__(self, env, handles, map_size, max_steps, models,
                play_handle, render=False, save_every=None, tau=None, log_name=None, data_dir=None, log_dir=None, model_dir=None, render_dir=None, train=False, vae_train=False):
        """Initialize runner
        ...
        # training trackors
        self.episode_return_trackor0=list() # episode return
        self.episode_return_trackor1=list() # episode return
    ...
    def run(self, variant_eps, iteration, data_dir=None,win_cnt=None):
        ...
        max_nums, nums, agent_r_records, total_rewards, render_list = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
                    models=self.models, print_every=50, eps=variant_eps, render=(iteration + 1) % self.save_every == 0 if self.save_every > 0 else False, train=True, vae_train=False)
        ...
        self.episode_return_trackor0.append(total_rewards[0])
        self.episode_return_trackor1.append(total_rewards[1])
        
        with open(os.path.join(data_dir,'episode_return0.json'),'w') as f:
            json.dump(str(self.episode_return_trackor0),f)
        with open(os.path.join(data_dir,'episode_return1.json'),'w') as f:
            json.dump(str(self.episode_return_trackor1),f)
    ...
    def run_vae(self, variant_eps, iteration, data_dir=None,win_cnt=None):

        max_nums, nums, agent_r_records, total_rewards, render_list = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
                    models=self.models, print_every=50, eps=variant_eps, render=(iteration + 1) % self.save_every == 0 if self.save_every > 0 else False, train=False, vae_train=True)
